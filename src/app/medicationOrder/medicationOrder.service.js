(function () {
    ...
    var serviceId = 'medicationOrderService';

    function medicationOrderService($filter, $http, $timeout, common, dataCache, fhirClient, fhirServers, localValueSets) {
        var dataCacheKey = 'localMedicationOrders';
        var itemCacheKey = 'contextMedicationOrder';
        var logError = common.logger.getLogFn(serviceId, 'error');
        var logInfo = common.logger.getLogFn(serviceId, 'info');
        var $q = common.$q;

        function addMedicationOrder(resource) {
            _prepArrays(resource);
            var deferred = $q.defer();
            fhirServers.getActiveServer()
                .then(function (server) {
                    var url = server.baseUrl + "/MedicationOrder";
        ...
        function deleteCachedMedicationOrder(hashKey, resourceId) {
            function removeFromCache(searchResults) {
                if (searchResults && searchResults.entry) {
                    var cachedMedicationOrders = searchResults.entry;
                    searchResults.entry = _.remove(cachedMedicationOrders, function (item) {
            ...
            var deferred = $q.defer();
            deleteMedicationOrder(resourceId)
        ...
        function deleteMedicationOrder(resourceId) {
            var deferred = $q.defer();
        ...
        function getCachedMedicationOrder(hashKey) {
            function getMedicationOrder(searchResults) {
                var cachedMedicationOrder;
                var cachedMedicationOrders = searchResults.entry;
                for (var i = 0, len = cachedMedicationOrders.length; i < len; i++) {
                    if (cachedMedicationOrders[i].$$hashKey === hashKey) {
                        cachedMedicationOrder = cachedMedicationOrders[i].resource;
                        var baseUrl = (searchResults.base || (activeServer.baseUrl + '/'));
                        cachedMedicationOrder.resourceId = (baseUrl + cachedMedicationOrder.resourceType + '/' + cachedMedicationOrder.id);
                        cachedMedicationOrder.hashKey = hashKey;
                        break;
                    }
                }
                if (cachedMedicationOrder) {
                    deferred.resolve(cachedMedicationOrder);
                } else {
                    deferred.reject('MedicationOrder not found in cache: ' + hashKey);
                }
            }
            ...
            getCachedSearchResults()
                ...
                .then(getMedicationOrder,
                function () {
                    deferred.reject('MedicationOrder search results not found in cache.');
                });
            return deferred.promise;
        }
        ...
        function getMedicationOrder(resourceId) {
            var deferred = $q.defer();
        ...
        function getMedicationOrderContext() {
            return dataCache.readFromCache(dataCacheKey);
        }

        function getMedicationOrderReference(baseUrl, input) {
            var deferred = $q.defer();
            fhirClient.getResource(baseUrl + '/MedicationOrder?code=' + input + '&_count=20')
                .then(function (results) {
                    var medicationOrders = [];
                    if (results.data.entry) {
                        angular.forEach(results.data.entry,
                            function (item) {
                                if (item.content && item.content.resourceType === 'MedicationOrder') {
                    ...
                    if (medicationOrders.length === 0) {
                        medicationOrders.push({display: "No matches", reference: ''});
                    }
                    deferred.resolve(medicationOrders);
                }, function (outcome) {
                    deferred.reject(outcome);
                });
            return deferred.promise;
        ...
        function searchMedicationOrders(baseUrl, searchFilter) {
            var deferred = $q.defer();

            if (angular.isUndefined(searchFilter) && angular.isUndefined(organizationId)) {
                deferred.reject('Invalid search input');
            }
            fhirClient.getResource(baseUrl + '/MedicationOrder?' + searchFilter + '&_count=20')
                .then(function (results) {
        ...
        function getMedicationOrders(baseUrl, searchFilter, patientId) {
            var deferred = $q.defer();
            var params = '';
            ...
            fhirClient.getResource(baseUrl + '/MedicationOrder?' + params + '&_count=20')
                .then(function (results) {
                    dataCache.addToCache(dataCacheKey, results.data);
                    deferred.resolve(results.data);
                }, function (outcome) {
        ...
        function getMedicationOrdersByLink(url) {
            var deferred = $q.defer();
        ...
        function initializeNewMedicationOrder() {
            return {
                ...
                // effective[x]: Physiologically Relevant time/time-period for medicationOrder. One of these 2:
                ...
                "identifier": [{
                    "system": "urn:fhir-cloud:medicationOrder",
                ...
                "related": [
                    ...
                    //  "target": null // Reference(MedicationOrder)
                ]
            };
        }

        function setMedicationOrderContext(data) {
            dataCache.addToCache(itemCacheKey, data);
        }

        function updateMedicationOrder(resourceVersionId, resource) {
            _prepArrays(resource);
            var deferred = $q.defer();
        ...
        function seedRandomMedicationOrders(organizationId, organizationName) {
            var deferred = $q.defer();
            var birthPlace = [];
            var mothersMaiden = [];
            $http.get('http://api.randomuser.me/?results=25&nat=us')
                .success(function (data) {
                    angular.forEach(data.results, function (result) {
                        ...
                        timer.then(function () {
                            addMedicationOrder(resource).then(function (results) {
                                logInfo("Created medicationOrder " + user.name.first + " " + user.name.last + " at " + (results.headers.location || results.headers["content-location"]), null, false);
                            }, function (error) {
                                logError("Failed to create medicationOrder " + user.name.first + " " + user.name.last, error, false);
                            })
                        })
                    });
                    deferred.resolve();
        ...
        function _randomMothersMaiden(array) {
            var extension = {
                "url": "http://hl7.org/fhir/StructureDefinition/medicationOrder-mothersMaidenName",
                "valueString": ''
            };
            if (array.length > 0) {
        ...
        var service = {
            addMedicationOrder: addMedicationOrder,
            clearCache: clearCache,
            deleteCachedMedicationOrder: deleteCachedMedicationOrder,
            deleteMedicationOrder: deleteMedicationOrder,
            getCachedMedicationOrder: getCachedMedicationOrder,
            getCachedSearchResults: getCachedSearchResults,
            getMedicationOrder: getMedicationOrder,
            getMedicationOrderContext: getMedicationOrderContext,
            getMedicationOrderReference: getMedicationOrderReference,
            getMedicationOrders: getMedicationOrders,
            getMedicationOrdersByLink: getMedicationOrdersByLink,
            initializeNewMedicationOrder: initializeNewMedicationOrder,
            setMedicationOrderContext: setMedicationOrderContext,
            updateMedicationOrder: updateMedicationOrder,
            seedRandomMedicationOrders: seedRandomMedicationOrders,
            searchMedicationOrders: searchMedicationOrders
        };
